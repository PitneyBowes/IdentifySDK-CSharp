...
{
    [TestClass]
    public class GetCityStateProvinceTest
    {
        ...
        public void Test()
        {
            try
            {
                ...
                rowList.Add(new Record(user_fields, postalCode: "11368"));

                List<user_field> user_fieldsample = new List<user_field>();
                rowList.Add(new Record(null, postalCode: "11208"));
                options op = new options();

                input input = new input();
                input.RecordList = rowList;

                GetCityStateProvinceAPIRequest request = new GetCityStateProvinceAPIRequest(input, op);
                GetCityStateProvinceAPIResponse response = identifyAddressService.GetCityStateProvince(request);

                Assert.IsInstanceOfType(response, typeof(GetCityStateProvinceAPIResponse));
                string output = Utility.ObjectToJson<GetCityStateProvinceAPIResponse>(response);
        ...
        public void TestAsync()
        {
            try
            {
                ...
                identifyAddressService.GetCityStateProvinceFinishedEvent += (object sender, WebResponseEventArgs<GetCityStateProvinceAPIResponse> eventArgs) =>
                {
                    try
                    {
                ...
                rowList.Add(new Record(user_fieldsample, postalCode: "11368"));
                rowList.Add(new Record(user_fieldsample, postalCode: "11208"));
                options op = new options();
                ...
                GetCityStateProvinceAPIRequest request = new GetCityStateProvinceAPIRequest(input, op);
                identifyAddressService.GetCityStateProvinceAsync(request);
        ...
        public void getRecordjsonTestAndCompareSDKResponseTest()
        {
            try
            {
                ...
                rowList.Add(new Record(null, postalCode: "11368"));
                ...
                GetCityStateProvinceAPIRequest request = new GetCityStateProvinceAPIRequest(input, op);
                GetCityStateProvinceAPIResponse response = identifyAddressService.GetCityStateProvince(request);
                string sdkResponse = Utility.ObjectToJson<GetCityStateProvinceAPIResponse>(response);

                String apiResponse = TestUtility.ValidateFromAPI(TEST_URL, "/identifyaddress/v1/rest/getcitystateprovince/", TEST_TOKEN, inputjsonRecords, com.pb.identify.utils.Utility.contentType.json);
